  ...
  const fetchPostData = async () => {
    try {
      ...
      [...userPostAccounts, ...allPostsMetadata]
        ...
        .forEach((data) => {
          commentMap.set(data.replyTo.toString(), [
            {
              from: data.profile,
              text: data.postData.data.content.content,
              cl_pubkey: data.cl_pubkey,
            },
            ...(commentMap.has(data.replyTo.toString())
              ? commentMap.get(data.replyTo.toString())
              : []),
          ]);
        });
      dispatch(updateCommentMap(commentMap));
      dispatch(
        updatePosts(
          [...userPostAccounts, ...allPostsMetadata]
            .filter((data) => {
            ...
            .map((data) => {
              ...
              return {
                ...
                replies: commentMap.has(postPubkey.toString())
                  ? commentMap.get(postPubkey.toString())
                  : [],
              };
            })
        )
        ...
        return [
          user.publicKey.toString(),
          {
            ...
            wallet: userMap.get(user.account.user.toString())
              ? new PublicKey(userMap.get(user.account.user.toString()))
              : PublicKey.default,
          },
        ];
      })
