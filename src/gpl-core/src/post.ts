...
export class Post {
   ...
   */
  public async getPostAccountsByUser(
    userPubKey: anchor.web3.PublicKey
  ): Promise<anchor.ProgramAccount<any>[]> {
    const profiles = await this.sdk.profile.getProfileAccountsByUser(
      userPubKey
    );
   ...
   * @deprecated This function is slow and may cause performance issues. Consider using getPostsByUser instead.
   */
  public async getPostAccounts(
    user?: anchor.web3.PublicKey,
    profile?: anchor.web3.PublicKey
  ): Promise<anchor.ProgramAccount<any>[]> {
    const post = await this.sdk.program.account.post.all(
      profile ? [{ memcmp: { offset: 8, bytes: profile.toBase58() } }] : []
    );
    return post;
  }
  ...
  public async update(
    ...
    return this.sdk.program.methods.updatePost(newMetadataUri).accounts({
      post: postAccount,
      profile: profileAccount,
      user: userAccount,
      authority: owner,
  ...
  public async getAllPosts(): Promise<GraphQLPost[]> {
    ...
    const data = await this.sdk.gqlClient.request<{
      gum_0_1_0_decoded_post: GraphQLPost[];
    }>(query);
  ...
  public async getPostsByUser(
    ...
    const query = gql`
      query GetPostsByUser {
        gum_0_1_0_decoded_post(where: {profile: {_in: [${profilePDAs
          .map((pda) => `"${pda}"`)
          .join(",")}] }}) {
          cl_pubkey
          metadata
  ...
  public async getPostsByProfile(
  ...
  public async getPostsByNamespace(
    namespace: Namespace
  ): Promise<GraphQLPost[]> {
  ...
  public async getPostsByFollowedUsers(
    profileAccount: anchor.web3.PublicKey
  ): Promise<GraphQLPost[]> {
    const followedUsersProfileAccounts =
      await this.sdk.connection.getFollowingsByProfile(profileAccount);
    const query = gql`
      query GetPostsByFollowedUsers {
        gum_0_1_0_decoded_post(where: {profile: {_in: [${followedUsersProfileAccounts
          .map((pda) => `"${pda}"`)
          .join(",")}] }}) {
          cl_pubkey
          metadata
  ...
  public async getFeedsByFollowedUsers(
    profileAccount: anchor.web3.PublicKey
  ): Promise<GraphQLFeed[]> {
    const followedUsersProfileAccounts =
      await this.sdk.connection.getFollowingsByProfile(profileAccount);
    ...
    const feed = await Promise.all(
      data.gum_0_1_0_decoded_post.map(async (post: GraphQLPost) => {
        const profileData =
          await this.sdk.profileMetadata.getProfileMetadataByProfile(
            new anchor.web3.PublicKey(post.profile)
          );
        return {
